...
// --- Yahoo Finance dynamic fetching utilities ---
// These helpers let the app load real data from Yahoo Finance endpoints.
// Keep existing mocks/hooks for fallback and dev usage.
...
const yahooQuoteUrl = (symbols: string[]) =>
  `/yahoo/v7/finance/quote?symbols=${encodeURIComponent(symbols.join(','))}`;

/**
 * Fetch live quotes from Yahoo Finance.
 * Note: Direct client requests may face CORS in some environments. If so, proxy this URL.
 */
export async function fetchYahooQuotes(symbols: string[]): Promise<Stock[]> {
  if (!symbols.length) return [];

  const response = await fetch(yahooQuoteUrl(symbols));
  if (!response.ok) {
    if (response.status === 429) {
      const retryAfterHeader = response.headers.get('Retry-After');
      const retryAfterSeconds = retryAfterHeader ? parseInt(retryAfterHeader, 10) : NaN;
      const retryAfterMs = Number.isFinite(retryAfterSeconds) ? retryAfterSeconds * 1000 : 60000;
      const error: Error & { status?: number; retryAfterMs?: number } = new Error(
        `Yahoo quote fetch rate-limited (429)`
      );
      error.status = 429;
      error.retryAfterMs = retryAfterMs;
      throw error;
    }
    throw new Error(`Yahoo quote fetch failed: ${response.status} ${response.statusText}`);
  }

  const data: YahooQuoteResponse = await response.json();
  const now = new Date();

  return (data.quoteResponse?.result || []).map((q): Stock => ({
    symbol: q.symbol,
...
/**
 * Poll Yahoo Finance for a set of symbols on an interval and return `Stock[]`.
 * Falls back to the last good value if a request fails.
 */
export function useYahooStocks(symbols: string[], pollIntervalMs = 10000) {
  const [stocks, setStocks] = useState<Stock[]>([]);

  useEffect(() => {
    let isMounted = true;
    let timeoutId: number | undefined;
    let nextDelay = Math.max(10000, pollIntervalMs);
    const MAX_BACKOFF_MS = 300000; // 5 minutes

    const load = async () => {
      try {
        const next = await fetchYahooQuotes(symbols);
        if (isMounted) setStocks(next);
        // reset backoff on success
        nextDelay = Math.max(10000, pollIntervalMs);
      } catch (_) {
        // On failure, use exponential backoff with jitter
        const err = _ as Error & { status?: number; retryAfterMs?: number };
        const base = err.status === 429 && err.retryAfterMs
          ? err.retryAfterMs
          : Math.min(nextDelay * 2, MAX_BACKOFF_MS);
        const jitter = Math.floor(base * (0.1 * Math.random()));
        nextDelay = Math.min(base + jitter, MAX_BACKOFF_MS);
      } finally {
        if (isMounted) {
          timeoutId = window.setTimeout(load, nextDelay);
        }
      }
    };
...
/**
 * Fetch closing price history for a symbol from Yahoo Finance chart API.
 * Returns an array of close prices (undefined values filtered out).
 */
export async function fetchYahooPriceHistory(
  ...
  const response = await fetch(yahooChartUrl(symbol, range, interval));
  if (!response.ok) {
    throw new Error(`Yahoo chart fetch failed: ${response.status} ${response.statusText}`);
  }
  const data: YahooChartResponse = await response.json();
  const series = data.chart?.result?.[0]?.indicators?.quote?.[0]?.close || [];
  return series.filter((v): v is number => typeof v === 'number' && !Number.isNaN(v));
}
